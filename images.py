...
def generate_image(champion: str, boots_item: Item, legendary_items: list,
                   summoner_spell_1: SummonerSpell, summoner_spell_2: SummonerSpell, skill_order: list,
                   primary_rune: PrimaryRune, rune_tree: RuneTree, lol_version: str):
    item_image_size = 64
    ...
    for idx, legendary_item in enumerate(legendary_items_images):
        template_image.paste(legendary_item, (outer_padding,
                                              champion_image_size + 2 * outer_padding + item_image_size * (
                                                          idx + 1) + line_height + inside_padding * (idx + 2)))
    ...
    for item in range(len(legendary_items)):
        create_item_label(legendary_items[item].en_name, template_image, 94, 202 + 74 * (item + 1))
        create_item_label(legendary_items[item].pl_name, template_image, 94, 202 + 24 + 74 * (item + 1))
